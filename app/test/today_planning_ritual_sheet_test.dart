...
void main() {
  testWidgets('Today can open planning ritual sheet', (tester) async {
    ...
    await tester.tap(
      find.byKey(const ValueKey('today_planning_ritual_action')),
    );
    await tester.pumpAndSettle();

    expect(find.text('计划仪式'), findsOneWidget);
    expect(find.text('1) 清理待处理'), findsOneWidget);
    expect(find.text('2) 装入今天计划'), findsOneWidget);
    expect(find.text('3) 进入专注'), findsOneWidget);
